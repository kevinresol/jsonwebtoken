...
';
	...
	public function testHS384() {
		...
		return signer.sign(cast {iss: 'iss'})
			.next(function(token:String) return assert('eyJhbGciOiJIUzM4NCIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJpc3MifQ.X9DaLysdsHc-zZJNEffbWd2HRrWmX3qDKToWGGIJc_0s2SIbQJdHDV804m8LDLSW' == token));
	}
	
	public function testHS512() {
		var signer = getSigner(HS512(secret));
		return signer.sign(cast {iss: 'iss'})
			.next(function(token:String) return assert('eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJpc3MifQ.ltlcf72E1-V-u2qGJ7MRiUBngFzp5vXrmC6wuKiQac8l6bcfVspLOnW_pCl-h5QiBp2ckxz51BAZKM8HQZ_-6Q' == token));
	}
	#end
